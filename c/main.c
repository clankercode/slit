...
static void passthrough_mode(struct slit_config *cfg) {
    ...
    if (n == 0) {
        /* -n 0: old pipe-everything behaviour */
    ...
    /* Phase 1: print first n lines, flushing each */
    ...
    /* Phase 2: buffer remaining lines in a circular array of capacity n */
    ...
    while (getline(&line, &len, stdin) != -1) {
        total++;
        if (tw) tee_line(tw, line);   /* tee ALL lines including skipped middle */
